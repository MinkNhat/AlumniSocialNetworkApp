    ...
    if (comment !== "") {
      ...
      if (editingCommentId != null) {
        let res = await authApis(token).patch(
          endpoints["comment-details"](editingCommentId),
          {
            content: comment,
          }
        );
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (newEvent.type === "update-comment") {
        setComments((prev) =>
          prev.map((comment) =>
            comment.id === newEvent.data.id
              ? { ...comment, content: newEvent.data.content }
              : comment
          )
        );
    ...
    <View style={styles.container}>
      ...
      {comments?.length === 0 && (
        <View style={styles.textComment}>
          <Text
            style={{ color: Theme.colors.text, marginLeft: 5, fontSize: 16 }}
          >
